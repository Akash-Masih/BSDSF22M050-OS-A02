/*
 * ls-v1.6.0.c
 * Enhanced LS command (v1.6.0)
 * Feature: Recursive Listing (-R) with Colorized Output
...
/* Function prototypes */
void listFiles(const char *path, bool longList, bool horizontal, bool recursive, bool colorize);
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "lRxv")) != -1) {
        switch (opt) {
            case 'l':
                longList = true;
                break;
            ...
            case 'x':
                ...
                break;
            case 'v':
                printf("Custom LS version 1.6.0 - Recursive & Colorized Output\n");
                return 0;
            default:
    ...
    listFiles(path, longList, horizontal, recursive, colorize);
    return 0;
}
...
void listFiles(const char *path, bool longList, bool horizontal, bool recursive, bool colorize) {
    ...
    if (!dir) {
        fprintf(stderr, "ls-v1.6.0: cannot open directory '%s': %s\n", path, strerror(errno));
        return;
    }

    printf("%s:\n", path); // Show directory header
    ...
    /* Display files */
    if (longList) {
        displayLongList(filenames, count, path, colorize);
    } else {
        displayInColumns(filenames, count, path, colorize);
    }
    ...
    if (recursive) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (S_ISDIR(statbuf.st_mode)) {
                listFiles(fullpath, longList, horizontal, recursive, colorize);
            }
        }
    }
...
void displayLongList(char **files, int count, const char *path, bool colorize) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("%s %2ld %2ld %6ld ", perms, (long)statbuf.st_nlink, (long)statbuf.st_uid, (long)statbuf.st_size);
        printFileWithColor(path, files[i]);
        printf("\n");
    }
}
